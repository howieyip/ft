...
class TradeOrderTest(ft.TradeOrderHandlerBase):
    def on_recv_rsp(self, rsp_pb):
        ...
        if data.order_status == ft.OrderStatus.FILLED_ALL:
            ...
            log.info('订单状态为%s' % data.order_status)
...
class TickerTest(ft.TickerHandlerBase):
    def on_recv_rsp(self, rsp_str):
        ...
        if not glb['over'] and (h < 11 or h == 11 and m < 55 or 13 <= h < 15 or h == 15 and m < 30):
...
def auto_adjust(delta_price, i, adjust_dict, submitted_type):
    ...
    if submitted_type.find('buy') > -1:
        rise_price = order_book['Bid'][0][0] - (adjust_dict['rise'][2] - 1) * 0.001
        fall_price = order_book['Bid'][0][0] - (adjust_dict['fall'][2] - 1) * 0.001
    elif submitted_type.find('sell') > -1:
        rise_price = order_book['Ask'][0][0] + (adjust_dict['rise'][2] - 1) * 0.001
        fall_price = order_book['Ask'][0][0] + (adjust_dict['fall'][2] - 1) * 0.001
...
def smart_buy(code, volume, price=None):
    if price is None:
    ...
    if data is not False and len(data) > 0:
        for i in range(0, len(data)):
            ...
            if data3 is False:
                ...
                time.sleep(1)
                data3 = smart_sell(data2.code, data2.qty)
            if data3 is not False and len(data3) > 0:
                data3 = data3.iloc[0]
...
def position_list_query(stock_type='', check=True, logging=True, reset=False):
    ...
    if len(data) > 0:
        for i in range(0, len(data)):
            set_has(data.iloc[i].code, data.iloc[i].stock_name)
...
def check_pl():
    if TRADE_ENV == ft.TrdEnv.SIMULATE:
        return False
    if glb['today_pl_val'] > glb['today_pl_max_val']:
        glb['today_pl_max_val'] = glb['today_pl_val']
